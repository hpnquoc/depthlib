...
class DepthProEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.patch_encoder = self._init_patch_enc(patch_encoder)
        self.image_encoder = self._init_image_enc(image_encoder)
        self.hook_block_ids = list(hook_block_ids)

        patch_encoder_embed_dim = self.patch_encoder.embed_dim
        image_encoder_embed_dim = self.image_encoder.embed_dim

        self.out_size = int(
            self.patch_encoder.patch_embed.img_size[0] // self.patch_encoder.patch_embed.patch_size[0]
        )
    ...
    def _init_patch_enc(self, patch_enc_conf: dict) -> nn.Module:
        return create_vit(**patch_enc_conf, use_pretrained=False)
    
    def _init_image_enc(self, image_enc_conf: dict) -> nn.Module:
        return create_vit(**image_enc_conf, use_pretrained=False)
