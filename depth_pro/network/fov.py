...
class FOVNetwork(nn.Module):
    ...
    def __init__(
        self,
        num_features: int,
        fov_encoder: Optional[dict] = None,
    ):
        ...
        if fov_encoder is not None:
            fov_encoder = self._init_fov_enc(fov_encoder)
    ...
    def _init_fov_enc(self, fov_enc_conf: dict) -> nn.Module:
        return create_vit(**fov_enc_conf, use_pretrained=False)
